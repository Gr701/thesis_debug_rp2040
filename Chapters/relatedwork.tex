...
The approach described above is implemented for C and C++ with GNU Debugger, further reffered as GDB. The first step in the procedure is to compile the program with the "-g" flag, which will make the compiler to generate besides the executable, a lookup table for the debugger, which maps the binary code instructions to the variables and the line numbers of the original code. Then the program is started as a child process of the debugger, so it can control the interrupts. 
