...
Arm Debug Interface (ADI) architecture provides an access to the debug components inside the SoC (System on a Chip). It consists of Access Ports (APs), connected directly to the resources like CPU debug registers, memory system, ROM table, and more; and Debug Port (DP), which serves as a facade of the system and has a physical connection to the debug probe \cite{adi}. DP provides a one standard way to access the information from different APs \cite{adi}. \reffig{fig:adiv5_implementation} illustrates the ADI version 5 with the block diagram. 

\subsection{SWD}

Serial Wire Debug (SWD) is the protocol defined by Arm as a part of the Arm Debug Interface (ADI) architecture \cite{adi}. 

SW-DP is one type of the physical connection to the debugger.   
