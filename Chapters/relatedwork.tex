...
\end{figure}
GDB's internal structure is described to contain two sides: the Symbol Side and the Target Side with the Interface level 
sitting on top to be the bridge between the developer and the program. 
...
used by Integrated Development Environments (IDEs) like Visual Studio Code (see Figure 1).
...
regardless of the host's Operating System (\texttt{OS}) or the program's binary and object 
files' formats [2]. After \texttt{BFD} has navigated the executable, the task is then 
...
The Target Side of \texttt{GDB} is in charge of the handling the program execution and raw data.
