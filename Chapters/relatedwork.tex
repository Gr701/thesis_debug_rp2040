...
The approach described above is implemented for C and C++ with GNU Debugger, further reffered as GDB. The first step in the procedure is to compile the program with "-g" flag, which besides the executable, will generate the lookup table for the debugger, mapping binary code instructions to the variables and the line numbers of the original code. Then the program is started as a child process of the debugger, so it can control the interrupts. 
