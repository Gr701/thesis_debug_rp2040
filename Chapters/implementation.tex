...
The approach where one core is used to debug another one was implemented and shared by majbthrd on Github. He developed two projects: "pico-debug" for RP2040 \cite{pico_debug}, and "pico2-debug" for RP2350 boards \cite{pico2_debug}. To setup the debugging session there are ready .uf2 files provided in the repositories. Pico-debug can be placed in RAM or cache memory of the board, and each option has its own .uf2 file. We are going with a more common and easier way to use it, placing it in the RAM, so the whole cahce is availiable for the code of the program that being debugged. To do so, we take the pico-debug-gimmecache.uf2 file from the releases page of the repository and flash it onto the board. 
